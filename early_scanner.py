...
def is_actual_stock(symbol, name):
    if not symbol or not name: return False
    ...
    if any(k in name for k in etf_keywords): return False
    
    warrant_suffixes = ['W', 'WS', 'WT', 'WR']
    if len(symbol) >= 2 and symbol[-1] in warrant_suffixes: return False
    if 'WARRANT' in name or 'RIGHT' in name: return False
    
    if any(ind in symbol or ind in name for ind in ['-P', 'PR', 'PREFERRED']): return False
    if symbol.endswith('U') and ('UNIT' in name or 'UNITS' in name): return False
    if any(k in name for k in ['BOND', 'NOTE', 'DEBT', 'SPAC', 'ACQUISITION']): return False
...
def save_stock_data_to_cache(symbol, stock_data):
    ...
    try:
        with open(get_cache_filename(symbol), 'w', encoding='utf-8') as f:
            json.dump({'cache_time': datetime.datetime.now().isoformat(), 'stock_data': stock_data}, f, ensure_ascii=False, indent=2)
    except:
        pass
...
def is_strong_stock(symbol, name, delisted_stocks=None):
    ...
    try:
        if USE_DATA_CACHE:
            ...
            if cached_result and cached_result.get('日期', '') == datetime.datetime.now().strftime('%Y-%m-%d'):
        ...
        # Fix: 使用 history 代替 download，避免 MultiIndex 问题
        df = pd.DataFrame()
        ...
        total_conditions = len(conditions) # 这里动态计算是 7
        
        # Fix: 取消总数必须等于 6 的硬编码限制
        if met_conditions == total_conditions:
            ...
            result = {
                ...
                "日期": info.get('firstTradeDateMilliseconds', 'N/A'),
            ...
            if USE_DATA_CACHE: 
                save_stock_data_to_cache(symbol, {"代码": symbol, "不符合条件": True, "满足条件": f"{met_conditions}/{total_conditions}"})
            return None
...
def scan_market():
    ...
    def sort_key(x):
        cap_str = x.get('市值', '')
        if 'B' in cap_str: return float(cap_str.replace('$', '').replace('B', '')) * 1e9
        if 'M' in cap_str: return float(cap_str.replace('$', '').replace('M', '')) * 1e6
        try: return float(cap_str.replace('$', '').replace(',', ''))
        except: return 0
